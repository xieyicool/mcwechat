function kalendar(id, dateTime, options) {
	var myOption = {
			dayClick: null
	}
	$.extend(myOption, options);
    //判断是否是闰年,计算每个月的天数
    ...
    function dateNoneParam() {
    	var day = dateTime ? new Date(dateTime): new Date();
        var today = new Array();
    ...
    if(myOption.dayClick){
    	dateDay.click(function(){
    		var date= new Date(yearChange, monthChange, $(this).text());
    		myOption.dayClick(date);
    	});
    }
}